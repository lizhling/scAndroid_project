...
import com.sunrise.micromarketing.entity.Message;
...
public class ScmbhcDbManager {
	...
	public ArrayList<BusinessMenu> searchBusinessMenuByKey(String key) {
		StringBuilder where = new StringBuilder();
		where.append(ScmbhcStore.BusinessMenu.NAME + " LIKE " + "'%" + key + "%'");
		where.append(" AND " + ScmbhcStore.BusinessMenu.MENU_TYPE + "<>" + BusinessMenu.MENU_TYPE);
		ArrayList<BusinessMenu> businessMenus = new ArrayList<BusinessMenu>();
		Cursor cursor = cr.query(ScmbhcStore.BusinessMenu.CONTENT_URI, sBusinessMenuProjection, where.toString(), null,
				ScmbhcStore.BusinessMenu.DEFAULT_SORT_ORDER);
		for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
			BusinessMenu businessMenu = cursorToBusinessBusinessMenu(cursor);
			businessMenus.add(businessMenu);
		}
		return businessMenus;
	}

	public ArrayList<BusinessMenu> queryBusinessMenu() {
		ArrayList<BusinessMenu> businessMenus = new ArrayList<BusinessMenu>();
	...
	public BusinessMenu queryBusinessMenuByPrcID(String prcID) {
		StringBuilder where = new StringBuilder();
		where.append(ScmbhcStore.BusinessMenu.PROD_PRCID).append(" like \'%" + prcID + "%\'");
		Cursor cursor = cr.query(ScmbhcStore.BusinessMenu.CONTENT_URI, sBusinessMenuProjection, where.toString(), null,
				ScmbhcStore.BusinessMenu.DEFAULT_SORT_ORDER);
		BusinessMenu businessMenu = null;
		if (cursor.getCount() > 0) {
	...
	public ArrayList<BusinessMenu> cursorToBusinessBusinessMenus(Cursor cursor) {
		ArrayList<BusinessMenu> messages = new ArrayList<BusinessMenu>();
		for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
			BusinessMenu businessMenu = cursorToBusinessBusinessMenu(cursor);
	...
	public synchronized void recordBusinessMenu(BusinessMenu businessMenu) {
		ContentValues values = BusinessMenuToContentValue(businessMenu);
		cr.insert(ScmbhcStore.BusinessMenu.CONTENT_URI, values);
	}

	public void recordBusinessMenuList(ArrayList<BusinessMenu> datas) {
		if (datas == null)
			return;
	...
	public synchronized void updateMenuBusiness(BusinessMenu businessMenu) {
		StringBuilder where = new StringBuilder();
	...
	public synchronized void deleteBusinessMenu(BusinessMenu businessMenu) {
		StringBuilder where = new StringBuilder();
		where.append(ScmbhcStore.BusinessMenu._ID + "==" + businessMenu.getId());
	...
	public synchronized void startDeleteBusinessMenu(AsyncQueryHandler handler, int token, BusinessMenu businessMenu) {
		StringBuilder where = new StringBuilder();
		where.append(ScmbhcStore.BusinessMenu._ID + "==" + businessMenu.getId());
	...
	public BusinessMenu cursorToBusinessBusinessMenu(Cursor cursor) {
	...
	private ContentValues BusinessMenuToContentValue(BusinessMenu businessMenu) {
