...
import com.sunrise.micromarketing.database.ScmbhcStore.BusinessMenu;
import com.sunrise.micromarketing.database.ScmbhcStore.Messages;
...
public class ScmbhcProvider extends ContentProvider {
	...
	public ScmbhcDBHelper openHelper = null;  
	
	private final static UriMatcher URI_MATCHER=new UriMatcher(UriMatcher.NO_MATCH);
	
	static{
		URI_MATCHER.addURI(ScmbhcStore.AUTHORIY, ScmbhcStore.BusinessMenu.ENTITY_NAME, BUSINESSMENU);
		URI_MATCHER.addURI(ScmbhcStore.AUTHORIY, ScmbhcStore.BusinessMenu.ENTITY_NAME+"/#", BUSINESSMENU_ID);
		URI_MATCHER.addURI(ScmbhcStore.AUTHORIY, "messages", MESSAGES);
		URI_MATCHER.addURI(ScmbhcStore.AUTHORIY, "messages/#", MESSAGE_ID);
	}
	...
	private Uri insertInternal(Uri uri, ContentValues initialValues){
		...
		case BUSINESSMENU:
		case BUSINESSMENU_ID:
			table=ScmbhcDBHelper.BUSINESSMEMNU_TABLE;
			nullColumnHack=BusinessMenu._ID;
			break;
		case MESSAGES:
		case MESSAGE_ID:
			table=ScmbhcDBHelper.MESSAGE_TABLE;
	...
	public boolean onCreate() {
		// TODO Auto-generated method stub
		openHelper=new ScmbhcDBHelper(this.getContext());
		return true;
	}
	...
	public Cursor query(Uri uri, String[] projectionIn, String selection, String[] selectionArgs,
		...
		case BUSINESSMENU:
		case BUSINESSMENU_ID:
			tables=ScmbhcDBHelper.BUSINESSMEMNU_TABLE;
            break;
		case MESSAGES:
		case MESSAGE_ID:
			tables=ScmbhcDBHelper.MESSAGE_TABLE;
            break;
		default:
            throw new UnsupportedOperationException("Invalid URI " + uri);
		}
    ...
    private void getTableAndWhere(Uri uri,int match,String userWhere,GetTableAndWhereOutParameter out){
    	...
    	switch(match){
    	case BUSINESSMENU:
    		  out.table = ScmbhcDBHelper.BUSINESSMEMNU_TABLE;
    		  break;
		case BUSINESSMENU_ID:
			  out.table = ScmbhcDBHelper.BUSINESSMEMNU_TABLE;
	             where = "_id=" + uri.getPathSegments().get(1);
		case MESSAGES:
  		  out.table = ScmbhcDBHelper.MESSAGE_TABLE;
  		  break;
		case MESSAGE_ID:
			  out.table = ScmbhcDBHelper.MESSAGE_TABLE;
	             where = "_id=" + uri.getPathSegments().get(1);
