...
public class AsyncImageLoader {
		...
		final Handler handler = new Handler() {
			public void handleMessage(android.os.Message msg) {
				...
				case DownLoadThread.DWONLOAD_FLIE_COMPLETE:
					...
					if (datas != null) {
						bitmap = BitmapFactory.decodeByteArray(datas, 0,
								datas.length);
					}
	...
	private void getImageRes(final String url, final Handler handler)
			throws IOException {
		String fileName = FileUtils.getFileNameByUrl(url);
		final String filename = fileName;
		boolean isExist = FileUtils.fileIsExist(
				ExtraKeyConstant.APP_SD_PATH_NAME, fileName);
		if (isExist) {
			new Thread() {
				public void run() {
					try {
						byte[] datas = FileUtils.readFiletoBytes(
								ExtraKeyConstant.APP_SD_PATH_NAME, filename);
						...
						if (isBitmapComplete) {
							Message msg = handler.obtainMessage(
									DownLoadThread.DWONLOAD_FLIE_COMPLETE,
									datas);
							handler.sendMessage(msg);
						} else {// 如果图片数据不全，删除文件，发送文件读取失败信息
							FileUtils
									.deleteFileByRelativePath(
											ExtraKeyConstant.APP_SD_PATH_NAME,
											filename);
							App.sDownFileManager.downFile(mContext, url,
									ExtraKeyConstant.APP_SD_PATH_NAME,
									filename, handler, !IsAlwaysFormNet, true);
						}
					} catch (IOException e) {
						e.printStackTrace();
					}
			...
			App.sDownFileManager.downFile(mContext, url,
					ExtraKeyConstant.APP_SD_PATH_NAME, filename, handler,
					!IsAlwaysFormNet, true);
		}
	}
