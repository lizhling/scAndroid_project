...
public class FileUtils {
	...
	/*
	 * public static String getSdCardPath() { return sdcaPath; }
	 */
	...
	public static File createSDFileByAbsPath(String path) {
...
//			return "file is exist";
	...
	public static void saveToFileByAbsPath(byte[] data, String path) {
		File file = createSDFileByAbsPath(path);
		FileOutputStream out = null;
		try {
			out = new FileOutputStream(file);
			out.write(data);
	...
	/*
	 * public static String getFileName(String url){ int
	 * index=url.lastIndexOf("/"); String fileName=url.substring(index+1);
	 * return fileName; }
	 */
	...
	public static String getTextFromAssets(Context context, String txtFileName, String charsetName) {
	...
	public static String getImageStr(String imgPath) {
		...
		// 读取图片字节数组
		try {
			String imgAbsPath = sdcaPath + imgPath;
			in = new FileInputStream(imgAbsPath);
	...
	/**
	 * 根据绝对路径，找到指定文件所在的绝对路径
	 * @param dir
	 * @param fileLastName
	 * @return
	 */
	public static String searchFileFormDir(String dir, String fileLastName) {
		...
		for (File f : list) {
			if (f.isDirectory()) {
				String result = searchFileFormDir(f.getAbsolutePath(), fileLastName);
				if (result != null) {
					return result;
				}
