	...
	private Handler mHandler = new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			case HANDLER_DOWN_APK_FAIL:
				...
				mDownApkProgressDialog.getButton(DialogInterface.BUTTON_POSITIVE).
...
private TaskListener mUpdateCommonResListener = new TaskListener() {
	...
	private void createUpdateApkVersionDialog() {
		CommonActions.createTwoBtnMsgDialog(this,
				...
				getString(R.string.update_version_info), 
				null, null, 
				new CommonActions.OnTwoBtnDialogHandler() {
	...
	private void handleTaskFailed(GenericTask task) {
		// TODO Auto-generated method stub
		CommonActions.createTwoBtnMsgDialog(this, null,
	...
	class UpdateCommonResTask extends GenericTask{
		...
		private void updateCommonRes(String commonResVersion)
				throws HttpException, BusinessException,
				ClientProtocolException, IOException {
			publishProgress(getString(R.string.downing_common_res));
			UpdateInfoBean updateInfoBean = CmucApplication.sServerClient
					.getUpdateInfoByOsInfo(commonResVersion,
							cmucApplication.getOsInfo(),
							UpdateInfoBean.COMMON_RES_CHECK_TYPE,
			...
			if (updateInfoBean.getUpdateType() == UpdateInfoBean.NO_NEED_UPDATE) {
				...
				if (!isExistCommonResDir) {
					downZipAndUnZip(downUrl,CmucApplication.APP_FILE_HTML_DATA_DIR,CmucApplication.COMMON_RES_ZIP_NAME,CmucApplication.APP_FILE_COMMON_RES_DATA_DIR);
				}
			} else {
				if (isExistCommonResDir)
					FileUtils.deleteDirByRelativePath(CmucApplication.APP_FILE_COMMON_RES_DATA_DIR);
					downZipAndUnZip(updateInfoBean.getDownloadUrl(),CmucApplication.APP_FILE_HTML_DATA_DIR,CmucApplication.COMMON_RES_ZIP_NAME,CmucApplication.APP_FILE_COMMON_RES_DATA_DIR);
					saveCommonInformation(updateInfoBean);
			}
		}
		
		private void updateBusinessGuide(String businessGuideResVersion)
				throws HttpException, BusinessException,
				ClientProtocolException, IOException {
			UpdateInfoBean updateInfoBean = CmucApplication.sServerClient
					.getUpdateInfoByOsInfo(businessGuideResVersion,
							cmucApplication.getOsInfo(),
							UpdateInfoBean.BUSINESSGUIDE_CHECK_TYPE,
							cmucApplication.getAppTag());
			...
			if (updateInfoBean.getUpdateType() == UpdateInfoBean.NO_NEED_UPDATE) {
				...
				if (!isExistBusinessGuideResDir) {
					downZipAndUnZip(downUrl,CmucApplication.APP_BUSINESS_GUIDE_TOP_DIR,CmucApplication.BUSINESS_GUIDE_ZIP_NAME,CmucApplication.APP_BUSINESS_GUIDE_DIR);
				}
				isBusinessGuidesUpdate=false;
			} else {
				if (isExistBusinessGuideResDir)
					FileUtils
							.deleteDirByRelativePath(CmucApplication.APP_BUSINESS_GUIDE_DIR);
				downZipAndUnZip(updateInfoBean.getDownloadUrl(),
						CmucApplication.APP_BUSINESS_GUIDE_TOP_DIR,
						CmucApplication.BUSINESS_GUIDE_ZIP_NAME,
		...
		private void updateIdentityVerification() throws HttpException, BusinessException{
			AppMenu appMenu=CmucApplication.sServerClient.getMenuDetail("SMRZ", ""+CmucApplication.IDENTITY_VERIFICATION_MENU_ID, cmucApplication.getScreenWidth()+"", cmucApplication.getScreenHeight()+"");
		    cmucApplication.setIdentityVerificationMenu(appMenu);
		}
		

		private void downZipAndUnZip(String downUrl,String destDir,String destFileName,String unzipDir) throws ClientProtocolException, IOException, HttpException{
	...
	private void createDownApkProgress() {
		...
		mDownApkProgressDialog.setButton(DialogInterface.BUTTON_POSITIVE,
				getString(R.string.background_donwn_apk),
