...
public class WebViewFragment extends Fragment {
	...
	private BroadcastReceiver mWebReciever = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(ActivityUtils.WEB_GO_BROADCAST)) {
				boolean currentFirstPage = intent.getBooleanExtra(
						ActivityUtils.FIRST_PAGE_EXTRAL, false);
				if (cmucApplication.isOpenWebViewInFragment()&&!isContainWebActivity) {
					isFirstPage=currentFirstPage;
					updateWebButtonBack();
					addWebWindow();
				}else{
					if(isFirstPage&&currentFirstPage){
						exitWebview(mActivity);
						removeWebWindow(mBusinessId);
					}
				}
				isFirstPage=currentFirstPage;
			}
	...
	public void refreshWebView() {
		if(mIsManualStart){
			...
			if(!isOpened(mBusinessId)){
				...
				WebViewWindow webViewWindow=getWebView(mBusinessId);
				isFirstPage=webViewWindow.isFirstPage();
				mWebView=webViewWindow.getWebView();
				mWebWindowPanel.addView(mWebView);
			}
			...
			public void onClick(View v) {
				...
				if(mUrl!=null){
					if(mIsBusinessWeb){
						clearWebDatas();
						try {
							CmucApplication.sDownFileManager.downFile(mActivity,mUrl, CmucApplication.APP_FILE_HTML_DATA_DIR,mFileName,mHandler,true,false);
						} catch (IOException e) {
	...
	private void setBusinessInformation() {
		if (mBusinessId != -1) {
			mBusinessInformation = new BusinessInformation(cmucApplication
					.getSettingsPreferences().getAuthentication(), mBusinessId
					+ "", cmucApplication.getPhoneIMEI(),
					cmucApplication.getPhoneIMSI(), mAppTag);
			mBusinessInformation.setPhoneNumber(cmucApplication.getSettingsPreferences().getMobile());
	...
	private void initUnZipPaths() {
		...
		try {
			...
			mZipFilePath = FileUtils.getAbsPath(CmucApplication.APP_FILE_HTML_DATA_DIR, mFileName);		
			String unTargetFolderName=mFileName.substring(0, mFileName.lastIndexOf("."));
			String unZipTargetFolderRelativePath=CmucApplication.APP_FILE_HTML_DATA_DIR+File.separator+unTargetFolderName;		
	...
	private void iniWebSetting() {
		...
		if(cmucApplication.isIsPad()||cmucApplication.getRealScreenWidth()>700)
			zoomDensity=WebSettings.ZoomDensity.FAR;
	...
	private void getHtml(final String url, final Handler handler) throws IOException{
		if(mTargetFolderIsExist){
			...
			CmucApplication.sDownFileManager.downFile(mActivity,url, CmucApplication.APP_FILE_HTML_DATA_DIR,mFileName,handler,true,false);
			}
		}else{
			mWebLoadStatus.setText("页面下载中0%");
			CmucApplication.sDownFileManager.downFile(mActivity,url, CmucApplication.APP_FILE_HTML_DATA_DIR,mFileName,handler,true,false);
	...
	/*public void setSubAccount(String mSubAccount) {
		this.mSubAccount = mSubAccount;
	}*/
	...
	private void back() {
		if(mLoadWebFailedPanel.getVisibility()==View.VISIBLE||mLoadProgressPanel.getVisibility()==View.VISIBLE){
			if (!cmucApplication.isOpenWebViewInFragment()) {
				exitWebview(mActivity);
				removeWebWindow(mBusinessId);
			}
