...
import com.starcpt.cmuc.utils.Base64;
 ...
 * @throws HttpException 此异常包装了�?��列底层异�?<br /><br />
 ...
 *             首先�?��request log, 确认不是人为错误导致请求失败</li>
 *             <li>HttpAuthException, 通常发生在Auth失败, �?��用于验证登录的用户名/密码/KEY�?/li>
 *             <li>HttpRefusedException, 通常发生在服务器接受到请�? 但拒绝请�? 可是多种原因, 具体原因
 *             服务器会返回拒绝理由, 调用HttpRefusedException#getError#getMessage查看</li>
 *             <li>HttpServerException, 通常发生在服务器发生错误�? �?��服务器端是否在正常提供服�?/li>
...
private String decodeJson(Response response) throws ResponseException {
	...
	String decodeJsonStr=ZipUtils.decodeBase64(jsonStr, Base64.PREFERRED_ENCODING, Base64.GZIP);
	return decodeJsonStr;
}

private URI convertRequestUrl(String url, List<NameValuePair> pairs)
		throws HttpException {
	String requestParamsStr=UrlUtils.buildQueryString(pairs);
	Log.d(TAG, "requestParamsStr:"+url+"?"+requestParamsStr);
    String reauestParamsByBase4=ZipUtils.encodeBase64(requestParamsStr, Base64.PREFERRED_ENCODING, Base64.GZIP);
    HashMap<String, String> requestPas=new HashMap<String, String>();
    requestPas.put("param", reauestParamsByBase4);
    String baseUrl=UrlUtils.buildUrlByQueryStringMapAndBaseUrl(url, requestPas);
...
private void handleResponseStatusCode(int statusCode, Response res)
    ...
    switch (statusCode) {
    case OK:
    	handleBusinessResultCode(statusCode,res);
        break;
...
private void handleBusinessResultCode(int statusCode,Response res) throws HttpException, BusinessException{
	...
	try {
		...
		throw new ResponseException("服务器繁忙，请稍后再试", statusCode);
	}
}
