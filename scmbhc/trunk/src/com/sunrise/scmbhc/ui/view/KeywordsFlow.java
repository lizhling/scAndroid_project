...
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener{
    ...
    /** 存储显示的关键字。 */  
    private Vector<SearchTag> vecKeywords;  
    ...
    private void init() {  
        ...
        random = new Random();  
        vecKeywords = new Vector<SearchTag>(MAX);  
    ...
    public boolean feedKeyword(SearchTag searchTag) {  
        boolean result = false;  
        if (vecKeywords.size() < MAX) {  
            result = vecKeywords.add(searchTag);  
        }  
        return result;  
    }  
    ...
    private void disapper() {  
        ...
        for (int i = size - 1; i >= 0; i--) {  
            ...
            // LogUtlis.showLogD("ANDROID_LAB", txt.getText() + " leftM=" +  
    ...
    private boolean show() {  
        if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {  
            ...
            for (int i = 0; i < size; i++) {  
                SearchTag keyword = vecKeywords.get(i);  
                // 随机颜色  
                ...
                txt.setText(keyword.getTagName()); 
                txt.setId(i);
...
//                txt.setBackgroundColor(ranColor);
                ...
                int strWidth=0;
				try {
					strWidth = (int) Math.ceil(paint.measureText(keyword.getTagName()));
				} catch (Exception e) {
					e.printStackTrace();
				}  
        ...
        for (int i = 0; i < size; i++) {  
            ...
            inner: for (int k = i - 1; k >= 0; k--) {  
                ...
                if (yDistance * (kXY[IDX_Y] - yCenter) > 0) {  
                    // LogUtlis.showLogD("ANDROID_LAB", "compare:" +  
                    ...
                    if (isXMixed(startX, endX, iXY[IDX_X], iXY[IDX_X] + iXY[IDX_TXT_LENGTH])) {  
                        ...
                        // LogUtlis.showLogD("ANDROID_LAB", "break");  
                        break inner;  
                    }  
                }  
            }  
    ...
    public SearchTag getSearchTag(int i){
    	return vecKeywords.get(i);
    }
