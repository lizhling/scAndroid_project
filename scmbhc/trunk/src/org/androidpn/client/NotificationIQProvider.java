    ...
    public IQ parseIQ(XmlPullParser parser) throws Exception {
        ...
        for (boolean done = false; !done;) {
            ...
            if (eventType == 2) {
                ...
                if ("optionType".equals(parser.getName())) {
                    notification.setOptionType(parser.nextText());
                }
                if ("optionContent".equals(parser.getName())) {
                    notification.setOptionContent(parser.nextText());
                    LogUtlis.showLogI("", "XmppManager:+++++optionContent:"+notification.getOptionContent());
                }
                if ("timestemp".equals(parser.getName())) {
                    notification.setTimestemp(parser.nextText());
                }
            } else if (eventType == 3
