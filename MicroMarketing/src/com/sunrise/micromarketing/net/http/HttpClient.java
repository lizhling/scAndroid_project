package com.sunrise.micromarketing.net.http;
...
import com.sunrise.micromarketing.exception.http.HttpAuthException;
import com.sunrise.micromarketing.exception.http.HttpException;
import com.sunrise.micromarketing.exception.http.HttpRefusedException;
import com.sunrise.micromarketing.exception.http.HttpServerException;
import com.sunrise.micromarketing.exception.http.HttpsClient.SSLSocketFactoryImp;
import com.sunrise.micromarketing.exception.logic.ServerInterfaceException;
import com.sunrise.micromarketing.net.http.utils.UrlUtils;
...
public class HttpClient {
	...
	private void prepareHttpClient() {
		...
		try {
			...
			final SSLSocketFactory sslSocketFactory = new SSLSocketFactoryImp(trustStore);// SSLSocketFactory.getSocketFactory();
			schemeRegistry.register(new Scheme("https", sslSocketFactory, 28081));
...
//			}
	...
	public String httpRequestStrWithoutKey(String url, List<NameValuePair> pairs) throws HttpException {

		if (pairs != null) {
			pairs.add(new BasicNameValuePair("timestep", System.currentTimeMillis() + ""));
		}
		URI uri = convertRequestUrlWithoutKey(url, pairs);
		return httpRequestStr(uri);
	}
	...
	private URI convertRequestUrlWithoutKey(String url, List<NameValuePair> pairs) throws HttpException {
		URI uri = createURI(url);
		if (pairs != null) {
			String requestParamsStr = UrlUtils.buildQueryString(pairs);
			LogUtlis.d(TAG, "requestParamsStr:" + url + "?" + requestParamsStr);
			String reauestParamsByBase4 = ZipAndBaseUtils.encodeBase64(requestParamsStr);
			String baseUrl = url + "?" + reauestParamsByBase4;
			uri = createURI(baseUrl);
		}
		return uri;
	}
